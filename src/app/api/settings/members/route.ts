...
export async function POST(request: NextRequest) {
  ...
  const member = await createMember({ name, position: position || '', department: department || '', role: role || 'member', pinHash: hashPin('1234'), notionPageId: '', notionToken: '' })
...
export async function PATCH(request: NextRequest) {
  ...
  if (session.role === 'leader' || session.role === 'admin') {
    ...
    if (notionToken !== undefined) updates.notionToken = notionToken
